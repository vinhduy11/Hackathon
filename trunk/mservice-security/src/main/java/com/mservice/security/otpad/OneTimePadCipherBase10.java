...
public class OneTimePadCipherBase10 {
	...
	private String internalEncrypt1(String key, String input) throws CipherException
	{
		if (input == null) {
			throw new CipherException("Cannot encrypt null plain text");
		}
		...
		{
			int msg = keyVals.get(input.charAt(i));
			int k = keyVals.get(key.charAt(i%key.length()));
			int val = mod(msg + k, BASE);
			char en =  valKeys.get(val);
			cipher += en;
			/*
	...
	public static String encrypt1(String key, String input) throws CipherException
	{
		return getInstance().internalEncrypt1(key, input);
	}
	private String internalDecrypt1(String key, String input) throws CipherException{
		if (input == null) {
			throw new CipherException("Cannot encrypt null plain text");
		}
		if (key == null) {
	...
	public static String decrypt1(String key, String input) throws CipherException
	{
		return getInstance().internalDecrypt1(key, input);
	}
