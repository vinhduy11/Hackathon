...
import com.mservice.momo.gateway.internal.soapin.information.SoapInProcessPlus;
...
/**
 * Created by concu on 9/26/15.
 */
public class NewSoapVerticle extends Verticle {
    //    public static final String ADDRESS = "com.mservice.momo.soapin";
    private Logger logger;
    private SoapInProcess mSoapProcessor;
    private SoapInProcessPlus mSoapProcessorPlus;
...
//    private DBProcess dbProcessPromotion;
    ...
    public void start() {
        ...
        mSoapProcessorPlus = new SoapInProcessPlus(logger, glbCfg);
    ...
    private void checkUserStatus(Message message) {
        MomoMessage momoMsg = MomoMessage.fromBuffer((Buffer) message.body());
        AgentResponseType agent = mSoapProcessorPlus.getAgentStatus("0" + momoMsg.cmdPhone);
    ...
    public void doAgentModifyExtra(Message message) {
        ...
        if (request != null) {
            Common.BuildLog log = new Common.BuildLog(logger);
            log.setPhoneNumber("0" + momoMsg.cmdPhone);
            StandardBizResponse response = mSoapProcessorPlus.modifyAgentInfoExtra(phoneNumber, keyValuePairList, log);
    ...
    public void doAgentModify(Message message) {
        ...
        if (request != null) {
            AgentInfo agentInfo = mSoapProcessorPlus.getAgentInfo(phoneNumber);
            if (agentInfo == null) {
                message.reply(false);
                return;
            }
            ...
            StandardBizResponse response = mSoapProcessorPlus.modifyAgentInfo(phoneNumber
                    , os.isReged
                    , os.isActivated
                    , request.getName()
                    , request.getDob()
            ...
            if (response != null && response.getResult() == 0) {
                ...
                if (isRetailer) {
                    final CreatesessionResponseType csrt = SessionManagerPlus.getCsrt(log.getTime());
                    if (csrt != null && !"".equalsIgnoreCase(csrt.getSessionid())) {
                        result = mSoapProcessorPlus.unMapNoNameGroup(csrt.getSessionid()
                                , phoneNumber
                                , noname_group_id
                                , log);
    ...
    public void getAgentInfo(Message message) {
        ...
        if (request != null && request.hasNumber()) {
            ...
            AgentInfo agentInfo = mSoapProcessorPlus.getAgentInfo(number);
    ...
    public void register(final Message message) {
        ...
        if (resultToClient == MomoProto.SystemError.ALL_OK_VALUE) {
            ...
            if ("1".equalsIgnoreCase(waitingReg)) {
                ...
                StandardBizResponse resetPinResp = mSoapProcessorPlus.resetPIN(number, register.getPin());
            ...
            AgentResponseType agent = mSoapProcessorPlus.getAgentStatus(number);
            ...
            if (objCoreStatus != null) {
                ...
                if (agentInfo != null) {
                    ...
                    log.writeLog();
                    mSoapProcessorPlus.modifyAgentInfo(number, true, true, agentInfo.getFullName(), agentInfo.getDateOfBirth(), agentInfo.getAddress(),
...
//                obj.setDateOfBirth(dob);
            ...
            if (mSoapProcessorPlus.doRegisterMS(obj
                    , arr_group
                    , arr_capset
                    , upperLimit
                    , "MoMoApp register"
    ...
    public void setZaloGroup(Message message) {
        ...
        groupResponse = mSoapProcessorPlus.mapAgentToGroup("88888888"
                , phone
                , groupId);

        StandardBizResponse mapAgentResp = mSoapProcessorPlus.mapAgentToCapset("88888888"
                , phone
                , capsetId
                , 1
                , upperLimit);
    ...
    public void setVisaGroup(Message message) {
        ...
        String phone = "0" + msg.cmdPhone;
        long groupId = DataUtil.stringToUNumber(visaGroup.getZaloGroup());
...
//        }
    ...
    public void logIn(Message message) {
        ...
        if (result == MomoProto.LogInReply.ResultCode.ALL_OK_VALUE) {
            ...
            SessionInfo sessionInfo = SessionManagerPlus.getCsrt(vertx, number, logIn.getMpin(), time);
            if (sessionInfo == null) {
...
//                }
                ...
                if (getBillInfoReq_tmp != null) {
                    ...
                    StandardMSResponse response = mSoapProcessorPlus.getBillInfo(number
...
//                }
                ...
                if (getBillInfoReq_tmp != null) {
                    ...
                    try {
                        jsonResult = mSoapProcessorPlus.getBillInfoByService(number
                                , getBillInfoReq_tmp.getProviderId()
                                , getBillInfoReq_tmp.getBillId());
                    } catch (Exception ex) {
                        jsonResult = null;
...
//        });
    ...
    public void doChangePin(Message message) {
        ...
        if (result != MomoProto.SystemError.SYSTEM_ERROR_VALUE) {
            ...
            StandardBizResponse res = mSoapProcessorPlus.changePIN(changePin.getNumber()
                    , changePin.getOldPin()
                    , changePin.getNewPin());
    ...
    public void doRecoveryPin(Message message) {
        ...
        if (result != MomoProto.SystemError.SYSTEM_ERROR_VALUE) {
            ...
            StandardBizResponse res = mSoapProcessorPlus.resetPINWithSms(recoveryNewPin.getNumber()
    ...
    private void LoadCfg(JsonObject config, Logger logger) {
        ...
        SessionManagerPlus.wsdl = config.getObject("soap_2").getString("soap_url_2", "http://172.16.18.50:8280/services/umarketsc?wsdl");
        SessionManagerPlus.initiator = config.getObject("soap_2").getString("soap_user_name_2", "admin_soapin");
        SessionManagerPlus.password = config.getObject("soap_2").getString("soap_password_2", "soap#access");
        SessionManagerPlus.sessionExpired = config.getObject("soap_2").getInteger("soap_session_expire_2", 6);
        SessionManagerPlus.logger = logger;
