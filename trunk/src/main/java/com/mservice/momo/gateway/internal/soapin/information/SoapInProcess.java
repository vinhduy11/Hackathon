...
//import apollo.backend.jdbc.DBProcessor;
...
public class SoapInProcess {
    ...
    public SoapInProcess(Logger logger, JsonObject glbCfg) {
        this.logger = logger;

        //for soapin
        this.wsdl = glbCfg.getObject("soap").getString("soap_url","http://172.16.18.50:8280/services/umarketsc?wsdl");
        ...
        this.soap_un =glbCfg.getObject("soap").getString("soap.un");
        this.soap_pw = glbCfg.getObject("soap").getString("soap.pw");
...
//	public CheckAccountResponse doCheckAccount(String agentReference, String mpin) {
//		System.out.println("doCheckAccount");
//		
//		if (stub == null)
//			callSoap();
//		CreatesessionResponseType csrt = null;
//		CheckAccountResponse agentInfo = null;
//		SessionInfo sessionInfo = SessionManager.getCsrt(agentReference, mpin, true);
//		int loginResultCode = sessionInfo.getLoginResult();
//		logger.debug("doCheckAccount- loginResultCode: " + loginResultCode);
//		if(loginResultCode == 0){
//			csrt = sessionInfo.getCsrt();
//		}else
//		{
//			agentInfo = new CheckAccountResponse();
//			agentInfo.setResultCode(ErrorMapping.getResultCode(loginResultCode));
//			agentInfo.setDescription(ErrorMapping.getDescriptionError(loginResultCode));
//			logger.debug("Return agentInfo with failure get sessionid sopain");
//			return agentInfo;
//		}
//		
//		if (csrt == null){
...
//			logger.debug("[checkAccountInfo] getAgentByReference Result: "+ agentResponseType.getResult());
//			logger.debug("[checkAccountInfo] getAgentByReference ResultName: "+ agentResponseType.getResultNamespace());
//			agentInfo = new CheckAccountResponse();
//			agentInfo.setResultCode(agentResponseType.getResult());
//			if (agentResponseType.getResult() == 0){
//				MarketAgent marketAgent = agentResponseType.getAgent();
//				marketAgent.getStatus();
//				List<KeyValuePair> agentData = marketAgent.getAgentData();
//				int length = agentData.size();
//				for (int i = 0; i < length; i++) {
//					KeyValuePair currAgentData = agentData.get(i);
//					logger.debug("[checkAccountInfo] get Value of key "
//							+ currAgentData.getKey() + "  : "
//							+ currAgentData.getValue());
//					if (currAgentData.getKey().equals("bank_code")) {
//						String bankCode = currAgentData.getValue();
//						agentInfo.setBankCode(bankCode);
//						agentInfo.setBankName(ErrorMapping.getBankName(bankCode));// Bank Name
//					}
//				}
//				switch (marketAgent.getStatus()) {
//				case 65:
//				case 1:
//					agentInfo.setStatus(1);// Registered, not active
//					break;
//				case 66:
//				case 2:
//					agentInfo.setStatus(1);// Not Registered, active
//					break;
//				case 67:
//				case 3:
//					agentInfo.setStatus(0);// Registered, Active
//					break;
//				default:
//					agentInfo.setStatus(1);// Not registered, not active
//					break;
//				}
//				long referenceID = marketAgent.getReferenceID();
//				GetAccountByReferenceIDRequest accountByReferenceIDAndTypeRequest = new GetAccountByReferenceIDRequest();
//				accountByReferenceIDAndTypeRequest.setReferenceID(referenceID);
//				accountByReferenceIDAndTypeRequest.setSessionid(csrt.getSessionid());
//				AccountsResponseType accountResponseType = stub.getAccountByReferenceID(accountByReferenceIDAndTypeRequest);
//
//				if (accountResponseType.getResult() == 0) {
//					List<Account> listAccount = accountResponseType
//							.getAccounts();
//					int accountLength = listAccount.size();
//					for (int i = 0; i < accountLength; i++) {
//						Account account = listAccount.get(i);
//						switch (account.getType()) {
//						case 1:
//							agentInfo.setMomoBalance(account.getDirtyBalance().getAvailableBalance().longValue());// MoMo
//							break;
//						case 2:
//							agentInfo.setMloadBalance(account.getDirtyBalance().getAvailableBalance().longValue());// MLoad
//							break;
//						default:
//							break;
//						}
...
//		}
//
//		logger.debug("Return agentInfo");
//		return agentInfo;
//	}



    /*public AgentInfo getAgentInfoWithoutMomo(String agentReference) {
        if (stub == null)
            callSoap();
        CreatesessionResponseType csrt = SessionManager.getCsrt();

        AgentInfo agentInfo = null;
        GetAgentByReferenceRequest agentByReferenceRequest = new GetAgentByReferenceRequest();
        logger.debug("getAgentInfo use SoapIn");
        ...
        if(agentResponseType.getResult() == 0){
            ...
            agentInfo.setCreated_date(DataUtil.xmlDateToString(marketAgent.getCreatedDate()));
    ...
    public AgentInfo getAgentInfo(String agentReference) {
        ...
        CreatesessionResponseType csrt = SessionManager.getCsrt(log.getTime());
        if(csrt==null){
            log.add("csrt","null");
        }
    ...
    public StandardMSResponse topupAirtime(Vertx _vertx, String agentReference
        ...
        SessionInfo sessionInfo = SessionManager.getCsrt(_vertx
                                                        ,agentReference
                                                        ,mpin
                                                        ,log.getTime());

        if(sessionInfo == null){
            log.add("SessionManager.getCsrt", "null");
            return null;
        }
        ...
        if (!"".equalsIgnoreCase(target))//phoneNumber hop le
        {
            ...
            KeyValuePair kvIsSMS = new KeyValuePair();
...
//            log.writeLog();
        ...
        KeyValuePair m2mTypeKp = new KeyValuePair();
        m2mTypeKp.setKey("m2mtype");
        m2mTypeKp.setValue(m2mType);

        KeyValuePair isSmsKp = new KeyValuePair();
    ...
    public StandardMSResponse transferMerchant(Vertx _vertx,String agentReference
            ,String mpin
            ,String phoneNumber
            ,long amount
            ,String notice
...
//        phoneNumber = "0" + DataUtil.strToInt(phoneNumber);
        ...
        SessionInfo sessionInfo = SessionManager.getCsrtForMerchantUser(_vertx
                ,agentReference
                ,mpin
                ,log.getTime());
        if(sessionInfo == null){
        ...
        if (loginResultCode == 0) {
            ...
            response = new StandardMSResponse();
            response.setResultCode(loginResultCode);
            response.setDescription(SoapError.CoreErrorMap_VN.get(loginResultCode));

            log.add("login result",loginResultCode);
            log.add("login desc",SoapError.CoreErrorMap_VN.get(loginResultCode));
//            log.writeLog();
            return response;
        }
    ...
    public StandardMSResponse billpay(Vertx _vertx,String agentReference
        ...
        StandardMSResponse response = new StandardMSResponse();
        SessionInfo sessionInfo = SessionManager.getCsrt(_vertx,agentReference, mpin,log.getTime());
        int loginResultCode = sessionInfo.getLoginResult();
        ...
        KeyValuePair account = new KeyValuePair();
        account.setKey("account");
        account.setValue(billId);

        KeyValuePair biller = new KeyValuePair();
        biller.setKey("billerid");
        biller.setValue(providerId);

        KeyValuePair _target = new KeyValuePair();
        ...
        } else //Cho phep check so tien no cua hoa don nay
        {
            ...
            try {
                ...
                if("aviettel".equalsIgnoreCase(providerId)){
                    ...
                    billInfo = "";
                    log.add("Response form soap: ",mapResponse.toString());
                    response.setDescription(mapResponse.toString());

                    if (mapResponse.containsKey("cusName")) {
                        billInfo += mapResponse.get("cusName");
                        log.add("cusName",billInfo);
                    }
...
//        System.out.println(phoneNumber);
    ...
    public StandardBizResponse resetPINWithSms(String agentReference, String newPin) {
        ...
        resetPinRequestType.setAgent(agentReference);
        resetPinRequestType.setNewPin(newPin);
        resetPinRequestType.setSuppressPinExpiry(false);
...
//        kvpm.getKeyValuePairs().add(kvIsSMS);
    ...
    public StandardBizResponse bankin(Vertx _vertx,String agent
        ...
        SessionInfo csrt = SessionManager.getCsrt(_vertx,agent, mpin,log.getTime());

        int loginResult;
        if(csrt== null){
            //khong login vao duoc soapin
            log.add("csrt","null");
            log.add("desc", "can not get session from SessionManager");
    ...
    public StandardBizResponse bankout(Vertx _vertx,String agent
        ...
        SessionInfo sessionInfo = SessionManager.getCsrt(_vertx, agent, mpin,log.getTime());
                                            ...
                                            ,Common.BuildLog log)
    {
        ...
        CreatesessionResponseType info = SessionManager.getCsrt(log.getTime());
