...
public class CoreMessage {
    ...
    public static  final CoreMessage fromBuffer(Buffer buffer){
        int dataLen = buffer.length();
        if (dataLen >= CoreMessage.MIN_LEN) {
            ...
            if (first == STX_BYTE) {
                ...
                if(len <= dataLen){
                    byte[] bdy = buffer.getBytes(CoreMessage.HEAD_LEN, len - 1);
                    return new CoreMessage(typ,idx,phn,bdy);
                }
                else{
    ...
    public static final Buffer buildBuffer(int type, long index, int phone, byte[] body){
        ...
        if(body == null){
            return new Buffer(CoreMessage.MIN_LEN)
                    .appendByte(STX_BYTE)
                    .appendInt(MIN_LEN)
                    .appendInt(type)
                    .appendLong(index)
            ...
            int len = body.length + CoreMessage.MIN_LEN;

            return new Buffer(MESSAGE_LEN)
                    .appendByte(STX_BYTE)
                    .appendInt(len)
                    .appendInt(type)
                    .appendLong(index)
    ...
    public Buffer toBuffer(){
        ...
        int len = ((cmdBody != null) ? cmdBody.length : 0) + CoreMessage.MIN_LEN;
        //len must smaller than 1024
        return new Buffer(MESSAGE_LEN)
                .appendByte(STX_BYTE)
                .appendInt(len)
    ...
    public CoreMessage(int type, long index, int phone, byte[] body){
        cmdType = type;
        cmdIndex = index;
        cmdPhone = phone;
        cmdBody = body;
    ...
    public static final int MESSAGE_LEN = 5*1024;
