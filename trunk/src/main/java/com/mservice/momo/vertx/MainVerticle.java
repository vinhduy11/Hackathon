...
public class MainVerticle extends Verticle {
    ...
    public void start() {
        ...
        if (jsonVerticles != null) {
            JsonArray jsonArrayVerticle = jsonVerticles.getArray(StringConstUtil.Verticle_json.VERTICLES, new JsonArray());

            Queue<JsonObject> queueVerticle = new ArrayDeque<>();
    ...
    private void deployVerticle(final Queue<JsonObject> queueVerticle) {
        if (queueVerticle.size() > 0) {
            ...
            final String nameOfVerticle = jsonVerticle.getString(StringConstUtil.Verticle_json.NAME, "");
            boolean isDeployed = jsonVerticle.getBoolean(StringConstUtil.Verticle_json.IS_DEPLOYED, true);
            if (isDeployed) {
                if (jsonVerticle.getBoolean(StringConstUtil.Verticle_json.IS_MODULE)) {
                    final String address = jsonVerticle.getString(StringConstUtil.Verticle_json.ADDRESS, "");
                    int instance = jsonVerticle.getInteger(StringConstUtil.Verticle_json.INSTANCES, 1);
                    int position = jsonVerticle.getInteger(StringConstUtil.Verticle_json.POSITION_OF_MODULE, 1);

                    JsonObject jsonConfig = returnJsonConfig(position);
                    container.deployModule(address, jsonConfig, instance, new Handler<AsyncResult<String>>() {
                        @Override
...
//                                deployVerticle(queueVerticle);
                ...
                } else if (jsonVerticle.getBoolean(StringConstUtil.Verticle_json.BLOCK)) {
                    String address = jsonVerticle.getString(StringConstUtil.Verticle_json.ADDRESS, "");
                    int instance = jsonVerticle.getInteger(StringConstUtil.Verticle_json.INSTANCES, 1);
                    boolean multiThread = jsonVerticle.getBoolean(StringConstUtil.Verticle_json.MULTITHREAD, true);
                    container.deployWorkerVerticle(address, globalCfg, instance, multiThread, new Handler<AsyncResult<String>>() {
                        @Override
                        public void handle(AsyncResult<String> asyncResult) {
                            if (asyncResult.succeeded()) {
                                logger.info(nameOfVerticle + " deploy successfully");
                                ...
                                logger.info(nameOfVerticle + " deploy fail");
                                logger.error(asyncResult.cause().getMessage());
                                deployVerticle(queueVerticle);
                            }
                        }
                    ...
                    int instance = jsonVerticle.getInteger(StringConstUtil.Verticle_json.INSTANCES, 1);
                    if (instance == 0) {
                        container.deployVerticle(address, globalCfg, new Handler<AsyncResult<String>>() {
                            @Override
                            public void handle(AsyncResult<String> asyncResult) {
                                if (asyncResult.succeeded()) {
                                    logger.info(nameOfVerticle + "deploy successfully");
                        ...
                        container.deployVerticle(address, globalCfg, instance, new Handler<AsyncResult<String>>() {
                            @Override
                            public void handle(AsyncResult<String> asyncResult) {
                                if (asyncResult.succeeded()) {
                                    logger.info(nameOfVerticle + "deploy successfully");
            ...
            if(globalCfg.getBoolean("isDeployedThreadBlock", false))
            {
                long timeBlock = globalCfg.getLong("isDeployedThreadBlockTime", 1000L * 60 * 5);
                vertx.setPeriodic(timeBlock, new Handler<Long>() {
                    @Override
                    public void handle(Long event) {
                        final long time = System.currentTimeMillis();
                        logger.info("Check ThreadBlock" + (System.currentTimeMillis() - time));
                        final Map<Thread,StackTraceElement[]> map = Thread.getAllStackTraces();
                        if(map.keySet() != null && map.keySet().size() > 0 && map.keySet().iterator() != null)
                        {
                            while (map.keySet().iterator().hasNext())
                            {
                                Thread thread = map.keySet().iterator().next();
                                if(thread.getName().startsWith("vert.x-eventloop-thread-")&& Thread.State.BLOCKED.equals(thread.getState()))
                                {
                            ...
                            logger.info("Eventloop threads checked. {} millis " + (System.currentTimeMillis() - time));
                        } else {
                            logger.info("map.keySize == 0");
                        }
                    }
                });
            }
...
//        else{
//            final AtomicLong sender = new AtomicLong(0);
//            final AtomicLong receiver = new AtomicLong(0);
//            final long[] ids = new long[1];
//            ids[0] = vertx.setPeriodic(5, new Handler<Long>() {
//                @Override
//                public void handle(Long aLong) {
//                    if(sender.incrementAndGet() >= 10000)
//                    {
//                        vertx.cancelTimer(ids[0]);
//                    }
//                    NewCoreConnectorVerticle.getBalance(vertx, "946810122", "111111", logger, new Handler<Integer>() {
//                        @Override
//                        public void handle(Integer integer) {
//                            logger.info("recever : " + receiver.incrementAndGet());
//                        }
//                    });
//                }
//            });
//        }
    ...
    private JsonObject returnJsonConfig(int number) {
        ...
        switch (number) {
            ...
            case 2:
                ...
                jsonObject = mongo_noti;
                break;
            case 3:
                JsonObject mysql_config = new JsonObject();
                mysql_config.putString("address", "io.vertx~mod-mysql-postgresql_2.10~0.3.1");
                mysql_config.putString("connection", "MySQL");
                mysql_config.putString("host", "localhost");
                mysql_config.putNumber("port", 3306);
                mysql_config.putNumber("maxPoolSize", 10);
                mysql_config.putString("username", "root");
                mysql_config.putString("password", "root");
                mysql_config.putString("database", "momovn_db_v1");
                mysql_config = globalCfg.getObject("mysql", mysql_config);
                jsonObject = mysql_config;
                break;
        }
